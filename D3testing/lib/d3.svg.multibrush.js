(function () {
	...
	function brushstart() {
		...
		// If the ALT key is down    center = null;
...
if (resizingX && move1(point, x, 0)) {
    ...
    function brushend() {
        ...
        g.style("pointer-events", "all").selectAll(".resize").style("display", function(d) { return brush.empty(d[1]) ? "none" :
		...
		if (!arguments.length) return extentent.map(function (d) {
	    ...
	    if (y) {
		if (yExtentDomain[0]) {
            yOutput = yExtente data-space extent to pixels.
		if (x) {
		    xOutput = z;
